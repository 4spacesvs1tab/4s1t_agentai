...
Security (KB-26-A):
  TOTP secrets are stored Fernet-encrypted in mfa_methods.secret.
  The Fernet key is derived from settings.SECRET_KEY via SHA-256.
  Backup codes are stored as SHA-256 hashes (one-time use, verified by re-hash).
  Existing plaintext secrets are transparently migrated on first verify.
"""
import pyotp
import secrets
import hashlib
...
def _get_fernet() -> Fernet:
    ...
    The 32-byte Fernet key is derived via SHA-256 of SECRET_KEY so we never
    store a raw key and the derivation is deterministic across restarts.
    """
...
def _encrypt_totp_secret(plain: str) -> str:
    """Encrypt a plaintext TOTP secret; returns a Fernet token (str)."""
...
def _decrypt_totp_secret(stored: str) -> str:
    ...
    Falls back to returning `stored` unchanged when the value is a legacy
    plaintext secret (not a Fernet token) so existing users are not locked
    out before a migration re-encrypts their secrets.
    """
    try:
        return _get_fernet().decrypt(stored.encode()).decode()
    except (InvalidToken, Exception):
        # Legacy plaintext — return as-is; will be re-encrypted on next enable_totp call
        return stored
...
class MFAService:
    ...
    def enable_totp_for_user(self, user_id: str, secret: str) -> bool:
        ...
        try:
            ...
            # Insert or update TOTP method — encrypt secret before storage (KB-26-A)
            ...
            """
            self.db.execute_command(insert_query, (mfa_id, user_id, MFAType.TOTP, encrypted_secret))
    ...
    def verify_totp_code(self, user_id: str, code: str) -> bool:
        ...
        try:
            ...
            stored_secret = result[0]["secret"]
            if not stored_secret:
                logger.warning(f"No TOTP secret found for user {user_id}")
                return False

            # Decrypt secret (KB-26-A); falls back to plaintext for legacy rows
            secret = _decrypt_totp_secret(stored_secret)
