...
class AuthService:
    ...
    def create_access_token(self, user_data: dict, expires_delta: Optional[timedelta] = None, token_type: str = "access", extra_claims: Optional[Dict[str, Any]] = None) -> str:
        ...
        Args:
            ...
            expires_delta: Token expiration time (default: ACCESS_TOKEN_EXPIRE_MINUTES from settings)
    ...
    def create_user(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            insert_query = """
                ...
                VALUES (hex(randomblob(16)), ?, ?, 'user', 1, datetime('now'))
            """
            ...
            # id is INTEGER (rowid alias) in the production schema — omit it so SQLite
            # auto-assigns; inserting hex(randomblob(16)) caused IntegrityError datatype mismatch.
            mfa_query = """
                INSERT INTO user_mfa (user_id, mfa_enabled, enrollment_complete,
