...
from config import kb_config as _kb_config
...
from agents.personas import get_persona, all_agent_types as _all_agent_types
...
_cfg = _get_agent_config().orchestrator

# Chars above which inter-wave results are compressed before the next wave.
_HANDOFF_TOKEN_THRESHOLD: int = _cfg.handoff_token_threshold
...
_TRIVIAL_MAX_LEN: int = _cfg.trivial_max_len
...
MAX_SUBTASKS: int = _cfg.max_subtasks
...
Rules:
- agent_type must be one of: {agent_types}
...
- Use research_agent for web research, information gathering, and KB queries.
- Use domain expert agents (any agent_type ending in _agent beyond the above) for domain-specific methodology, standards, and knowledge base queries.
- Use data_agent for data manipulation, Python analysis, visualisation.
- Use synthesis_agent ONLY when prior subtasks have already gathered data — it has no web_search access. NEVER route a standalone information-gathering or web-search task to synthesis_agent.
- For a simple single-step task, output exactly one subtask.
- Do NOT wrap the JSON in markdown code blocks.

KB routing rules (G1 — domain-to-topic mapping):
{kb_routing_rules}
- A task may span multiple domains — decompose into parallel research_agent subtasks, one per domain.

Source-mode rule: if the task starts with "[SOURCE: Use web search" or "[SOURCE: Use both knowledge_base_search AND web search", the primary research subtask MUST be routed to research_agent (which has web_search access). Never use synthesis_agent as the sole agent in these cases.

KB-first heuristic (G13): if the task asks about "my sources", "what did [person] say", \
"what do followed accounts think", or references a known account name, route to research_agent \
with knowledge_base_search — not web_search. Web search is a supplement when KB is insufficient.

Data + KB split rule (G20): if the task combines data analysis with opinions from followed sources \
(e.g. "analyse this CSV and tell me what Snider would say"), ALWAYS decompose into:
  - data_agent subtask for the data analysis
  - research_agent subtask for the KB/opinion query (parallel)
  - synthesis_agent subtask to combine results (depends on both)
Never route a task that needs both data analysis and KB lookup to data_agent alone — \
knowledge_base_search is not available to data_agent.\
"""
...
class OrchestratorAgent:
    ...
    async def run(self, task: str, context: str = "") -> AgentResult:
        ...
        if not self._is_complex(task):
            ...
            # Force the fast model — avoids routing trivial chat through a slow
            # thinking model even when the user has selected one as their default.
    ...
    async def _decompose_task(
        ...
        messages: list[dict] = [
            {"role": "system", "content": _DECOMPOSE_SYSTEM_PROMPT
                .replace("{kb_routing_rules}", _kb_config.get_routing_rules_text())
                .replace("{agent_types}", ", ".join(_all_agent_types()))
            },
            {
                "role": "user",
                "content": (
        ...
        for attempt in range(_MAX_DECOMPOSE_RETRIES + 1):
            ...
            try:
                ...
                logger.warning(
                    f"[orchestrator] decompose_task transient LLM failure: {exc}"
                )
                return None
            except Exception as exc:
                logger.error(
                    f"[orchestrator] decompose_task LLM call failed: {exc}",
                    exc_info=True,
                )
                return None
    ...
    async def _spawn_agent(
        ...
        Args:
            ...
            model_preference: Override the persona's default model preference slot.
        """
        try:
        ...
        agent = BaseAgent(
            ...
            model_preference_override=model_preference,
            extra_skill_grants=self._extra_skill_grants,
        )
    ...
    async def _summarise_for_handoff(
        ...
        try:
            ...
            logger.warning(f"[orchestrator] summarise_for_handoff transient failure: {exc}")
            return text[:_HANDOFF_TOKEN_THRESHOLD]
        except Exception as exc:
            logger.error(
                f"[orchestrator] summarise_for_handoff failed: {exc}",
                exc_info=True,
            )
