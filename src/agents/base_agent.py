...
_MAX_AGENT_STEPS = 20   # safety ceiling on tool-call iterations per agent run
...
class BaseAgent:
    ...
    async def run(self, task: str, context: str = "") -> AgentResult:
        ...
        model_id = self._model_selector.select(self._persona.model_preference)
        ...
        tools = self._registry.tools_for_agent(agent_type)
    ...
    async def _execute_tool_call(
        ...
        if skill_name not in self._persona.allowed_skills:
            err = (
    ...
    async def _maybe_compact_context(self, model_id: str) -> None:
        """
        Summarise the middle portion of message history when approaching 80% of
        the model's context token budget.

        Keeps: messages[0] (system prompt) + messages[-5:] (last 5 turns).
        ...
        if current < int(limit * 0.80):
            return

        # Need at least system + 5 turns + something in the middle to compact
        if len(self._messages) <= 7:
            return

        system_msg = self._messages[0]
        last_five = self._messages[-5:]
        middle = self._messages[1:-5]
        ...
        logger.info(
            f"[{self._persona.agent_type}] Compacting context: "
            f"{len(middle)} messages → 1 summary  "
            f"(tokens ~{current}/{limit})"
        )
