...
def get_privacy_config(yaml_path: Optional[Path] = None) -> PrivacyConfig:
    ...
    try:
        ...
        logger.warning("pyyaml not installed — using default PrivacyConfig")
    ...
    try:
        ...
        logger.warning(f"privacy.yaml not found at {path} — using defaults")
        return PrivacyConfig()
    except Exception as exc:
        logger.error(f"Failed to parse privacy.yaml: {exc} — using defaults")
        return PrivacyConfig()
