...
def _load_yaml(path: Path) -> dict:
    """Load providers.yaml. Returns empty dict on any error."""
    try:
        import yaml  # pyyaml
    except ImportError:
        logger.error(
            "pyyaml is not installed — cannot load providers.yaml. "
    ...
    try:
        ...
        logger.error(f"providers.yaml not found at {path}")
        return {}
    except Exception as exc:
        logger.error(f"Failed to parse providers.yaml: {exc}")
        return {}
...
def load_all_providers(
    ...
    """
    data = _load_yaml(yaml_path or _PROVIDERS_YAML)
