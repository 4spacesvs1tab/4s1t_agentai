...
class SystemInitializer:
    ...
    async def initialize_all(self) -> None:
        ...
        # Wire KB repository adapters at the composition root (E2).
        # Each domain singleton is created once here with an explicit SQLite
        # repo rather than relying on the lazy inline fallback.
        from core.db_path import get_db_path as _get_db_path
        ...
        from kb.social_graph import SocialGraph, get_social_graph
        from kb.alert_engine import AlertEngine, get_alert_engine
        from kb.discovery import DiscoveryManager, get_discovery_manager
        import kb.social_graph as _sg_mod
        import kb.alert_engine as _ae_mod
        import kb.discovery as _dm_mod

        _db_path = str(_get_db_path())
        _sg_mod._graph = SocialGraph(
            db_path=_db_path,
            account_repo=SqliteAccountRepository(_db_path),
        )
        _ae_mod._engine = AlertEngine(
            db_path=_db_path,
            alert_repo=SqliteAlertRepository(_db_path),
        )
        _dm_mod._manager = DiscoveryManager(
            db_path=_db_path,
            discovery_repo=SqliteDiscoveryRepository(_db_path),
        )
        logger.info("KB repository adapters wired at composition root")

        # Wire ContentIngested domain event subscribers (E4).
        # The EventBus uses string event types; handlers accept ContentIngested
        # domain objects. A thin adapter converts Event.payload → ContentIngested.
        from components.events.event_bus import get_event_bus
        from kb.domain.events import ContentIngested
        from kb.entity_extractor import EntityExtractor
        import kb.entity_extractor as _ee_mod

        _ee_api_key = __import__("os").environ.get("NANO_GPT_API_KEY", "")
        _ee_mod._extractor = EntityExtractor(api_key=_ee_api_key)
        _entity_extractor = _ee_mod._extractor

        def _ci_adapter(handler_fn):
            """Wrap a ContentIngested handler to work with the string-typed EventBus."""
            def wrapper(event):
                domain_event = ContentIngested(**event.payload)
                handler_fn(domain_event)
            return wrapper

        _event_bus = get_event_bus()
        _alert_engine = _ae_mod._engine
        _discovery_manager = _dm_mod._manager

        _event_bus.subscribe(
            "content_ingested",
            _ci_adapter(_alert_engine.handle_content_ingested),
        )
        _event_bus.subscribe(
            "content_ingested",
            _ci_adapter(_entity_extractor.handle_content_ingested),
        )
        _event_bus.subscribe(
            "content_ingested",
            _ci_adapter(_discovery_manager.handle_content_ingested),
        )
        logger.info("ContentIngested subscribers registered: alert_engine, entity_extractor, discovery_manager")
